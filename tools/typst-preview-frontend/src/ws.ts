...
export async function wsMain({ url, previewMode, isContentPreview }: WsArgs) {
  ...
  function createSvgDocument(kModule: RenderSession) {
    ...
    subsribes.push(
      fromEvent(window, "resize").subscribe(() => svgDoc.addViewportChange())
    );
    ...
    if (previewMode === PreviewMode.Slide) {
      {
        const inpPageSelector = document.getElementById(
          "typst-page-selector"
      ...
      const pagePrevSelector = document.getElementById(
        "typst-page-prev-selector"
      );
      if (pagePrevSelector) {
        pagePrevSelector.addEventListener("click", updatePrev);
      }
      const pageNextSelector = document.getElementById(
        "typst-page-next-selector"
      );
      if (pageNextSelector) {
...
function ensureInvertColors(
  root: HTMLElement | null,
  strategy: StrategyKey | StrategyMap
) {
  if (!root) {
    return;
  }
  ...
  function determineInvertColor() {
    ...
    if (vscodeAPI) {
      ...
      if (themeIsDark) {
        console.log(
          "invert-colors because detected by vscode theme",
      ...
      if (window.matchMedia("(prefers-color-scheme: dark)").matches) {
        console.log(
          "invert-colors because detected by (prefers-color-scheme: dark)"
        );
        return true;
      }
    }
